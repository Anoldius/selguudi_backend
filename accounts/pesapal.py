...
def get_pesapal_token():
    ...
    try:
        # verify=False inazuia timeout za SSL certificates kwenye Render Sandbox
        response = requests.post(url, json=payload, headers=headers, timeout=15, verify=False)
        print("PesaPal Token Status Code:", response.status_code)
        if response.status_code == 200:
            return response.json().get('token')
        else:
            print("PesaPal Token Error Body:", response.text)
    except Exception as e:
        print("PesaPal Token Request Exception:", str(e))
    return None
...
def register_pesapal_ipn(token, ipn_url):
    ...
    try:
        ...
        print("PesaPal IPN Register Exception:", str(e))
    return None
...
def submit_pesapal_order(token, order_payload):
    ...
    try:
        response = requests.post(url, json=order_payload, headers=headers, timeout=15, verify=False)
        print("PesaPal Order Status Code:", response.status_code)
        if response.status_code == 200:
            return response.json()
        else:
            print("PesaPal Order Error Body:", response.text)
    except Exception as e:
        print("PesaPal Submit Order Exception:", str(e))
    return None